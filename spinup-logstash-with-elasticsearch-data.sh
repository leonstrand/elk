...
# spin up dedicated elasticsearch data container
name_elasticsearch='elasticsearch'
elasticsearch_cluster_name='elasticsearch-mede'

echo
echo
next_container_elasticsearch_data=$next_container'-elasticsearch-data'
echo $0: info: elasticsearch data container name: $next_container_elasticsearch_data
...
echo
echo
echo $0: info: starting container $next_container_elasticsearch_data
# TODO node.ingest: true 
# TODO search.remote.connect: false
command="
docker run -d \
  --name $next_container_elasticsearch_data \
  ...
  -Enode.name=$(hostname)-$next_container_elasticsearch_data \
  -Enode.master=false \
  -Enode.data=true \
...
echo
echo
echo $0: info: waiting for $next_container_elasticsearch_data container on http port $next_http_port to respond with status 200
echo docker ps -f name=$next_container_elasticsearch_data
docker ps -f name=$next_container_elasticsearch_data
echo
...
if curl $ip:$next_http_port 1>/dev/null 2>&1; then
  ...
  echo $0: info: registering elasticsearch data transport service with consul
  ...
  # register es http
  echo
  echo $0: info: registering elasticsearch data http service with consul
  echo curl -v --retry 7 --retry-delay 3 \
  http://$CONSUL_IP:$CONSUL_PORT/v1/agent/service/register \
  -d "$(printf \''{
    "ID":"%s",
...
echo
echo
echo $0: info: waiting for elasticsearch data container to pass consul check
